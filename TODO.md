# MENSAENA – TODO
> Aktualisiert: 2026-06-12 (Design-Skills global installiert + Phase-5-Rest aufs Gate + Motion-Vorschlagspaket)
> JEDER Prompt = diese Datei updaten. KEINE AUSNAHME.
> [x]=done []=open [SQL]=User führt SQL aus [!]=kritisch

## Ad-hoc (2026-06-13)
- [x] sb.from-Abbau Batch 15 (Posts-Domäne): +4 PostsRepository-Methoden — listByModule (Modul-Feed mit module_key-Backwards-compat), create (vollständiger Insert, gibt ID zurück), listMyDrafts, suggestedTags (post_tags-Aggregation). delete existierte bereits. 4 Dateien Supabase-frei: module_posts_screen (1->0), module_create_post (1->0 Insert), drafts_screen (2->0 Liste+Delete), followed_tags_screen (1->0). Baseline: 19 Zugriffe / 13 Dateien (-83% seit Start 113/61). sb.storage in module_create bleibt (Bild-Upload, nicht Guard-relevant).
- [x] [mandatory] VERSION-BUMP 4.4.0+40400 -> 4.4.1+40401 — Sammel-Pflicht-Release fuer ALLE seit 4.4.0 gemergten Aenderungen (PRs #671-678): Design-/Motion-Skills global (13), EffectsGate-Rest, Motion-Pakete A-E (LevelUpCeremony/FirstHelpMoment/LivingFlame/CountUpText/MorphingActionButton/Map-Tiefe/Mensa-Pulse), C3 SkyBody-Bogen + B1 Pressable-Sweep, M5 Error-States, sb.from-Abbau Batches 11-14 (Calls/Krisen/Chat/Marketplace+Knowledge, Baseline 113/61 -> 24/17 = -79%), crisis_updates-Insert-Bugfix, OrbitViewer-Asset-Pipeline. Alles reiner Dart-/i18n-Code -> ging bereits per Shorebird-OTA (kumulativer Patch); der Bump erzwingt zusaetzlich die einheitliche APK-Auslieferung (flutter.yml erkennt Bump -> mandatory=true, shorebird_patch.yml skippt). Begruendung: User-Wunsch nach garantierter Voll-Auslieferung an alle Nutzer.
- [x] Motion C3 (SkyBody-Bogen) + B1-Sweep Fortsetzung (OTA, kein Bump): C3 — cinema_overlay hält die SkyBody jetzt persistent (stabiler Key statt phase-AnimatedSwitcher); SkyBody animiert den Tageszeit-Phasenwechsel selbst als Bogen über den Himmel (quadratische Bézier mit angehobenem Kontrollpunkt, easeInOut 1,4s) + Crossfade Farbe/Größe/Mond⇆Sonne. Nur auf full (intensity>=0.6), reduced/none schnappt hart; Guard auf alignment-Wechsel (stabiler specFor-Lookup → nur echte Phasenwechsel lösen aus). B1 — Pressable-Sweep auf 4 weitere Karten (smart_match, on_this_day, nearby_neighbors, friend_suggestions); today_events bleibt InkWell (Listen-Row). B3 (Pull-to-Refresh-Branding) BEWUSST NICHT umgesetzt — Emil-Prinzip: häufige Aktion (mehrmals/Tag) → keine aufwändige Marken-Animation; der vorhandene getönte RefreshIndicator ist die richtige zurückhaltende Wahl. Verbleibend (braucht Bump/Assets): A1 animations-Paket, C4 Shader 2+3, C5 OrbitViewer-Frames, C3-SkyBody steht.

## Ad-hoc (2026-06-12)
- [x] sb.from-Abbau Batch 14 (Marketplace+Knowledge): MarketplaceRepository.updateImages/requestReservation NEU; KnowledgeRepository ausgebaut (listPublic[edu-Filter], publish, recordRead — vorher nur uploadImage). 4 Dateien Supabase-frei: marketplace_create (1->0 Bild-Patch), marketplace_detail (1->0 Reservierungs-Wunsch), knowledge_screen (2->0 Liste + Lese-Tracker), knowledge_create (1->0 Veröffentlichen). Baseline: 24 Zugriffe / 17 Dateien (-79% seit Start 113/61). OFFEN als eigener Batch: module_posts/module_create (Posts-Domäne, großer Insert).
- [x] sb.from-Abbau Batch 13 (Chat-Domäne): +6 Repo-Methoden — ProfilesRepository.searchMentions/searchForNewChat/idByDisplayName, ConversationsRepository.myLastReadAt/recentContents, DmCallService.peerOf. 4 Dateien Supabase-frei: chat_screen (2->0: @-Mention-Suche + last_read_at beim Öffnen), chat_message_bubble (2->0: Zurückrufen-Peer via DmCallService.peerOf, @-Mention-Tap via idByDisplayName), chat_media_sheet (1->0: Medien-Liste via recentContents), messages_screen (1->0: Personensuche für neue DM, supabase_service-Import entfernt). sb.channel/sb.auth im chat_screen bleiben (Realtime/Auth, nicht Guard-relevant). Baseline: 29 Zugriffe / 21 Dateien (-74% seit Start 113/61).
- [x] sb.from-Abbau Batch 12 (Krisen-Domäne): CrisisRepository +4 Methoden (latestCriticalActive, myRecentSafeCheckin, insertSafeCheckin, setMyUserStatus). 5 Dateien Supabase-frei: safe_checkin_button (3->0), sos_button (2->0, auf EmergencyNumber-Modell + EmergencyNumbersRepository.listForCountry mit DE-Fallback umgestellt), critical_crisis_alert_listener (1->0), crisis_dashboard (1->0), crisis_detail (1->0). BUGFIX dabei: crisis_detail-Update-Sheet insertete mit user_id/severity — crisis_updates hat aber author_id/update_type (CrisisUpdate-Modell, fromJson crasht bei author_id=null) → jetzt CrisisRepository.addUpdate mit Mapping info/progress/resolved -> info/status_change/resolution. Baseline: 35 Zugriffe / 25 Dateien (-69% seit Start 113/61).
- [x] sb.from-Abbau Batch 11 (Calls-Domäne): NEU repositories/calls_repository.dart (scheduledUpcoming/scheduleCreate/scheduleCancel, voicemailsForMe/voicemailMarkListened, history[Join+Embed-Fallback]); DmCallService.watchIncomingFor NEU (Realtime-Stream der eingehenden Calls, limit 20). 4 Dateien Supabase-frei: scheduled_calls_screen (3->0), incoming_call_listener (3->0, Quick-Reply via MessagesRepository.send [jetzt korrekt mit updated_at-Touch], Caller-Lookup via ProfilesRepository.getById), voicemail_screen (2->0), call_history_screen (2->0). Baseline: 43 Zugriffe / 30 Dateien (-62% seit Start 113/61). sb.auth im Listener bleibt (Auth, nicht Guard-relevant).
- [x] M5 Error-States (Audit-Folgepaket, abgeschlossen): global_search zeigt bei Such-Fehler jetzt ErrorRetryCard mit Retry statt irreführendem "keine Ergebnisse"; chat_live_banner + Pinned-Panel mit explizitem hasError→shrink (Schmuck-Banner blockieren nie). BEFUNDE: notifications_screen + livestream_chat(+resolver) hatten bereits error-Branches — Audit-Punkte überholt.
- [x] C2 Parallax-Header-Adoption: BEFUND — bereits flächendeckend adoptiert (marketplace_detail, event_detail, group_detail via ParallaxImageHeader; post_detail eigener Parallax-Hero; organization_detail TiltCard-Cover). Kein Handlungsbedarf, aus der Motion-Rest-Liste gestrichen.
- [x] Motion-Pakete A–E umgesetzt (alle OTA-fähigen Teile, reine Dart-Änderungen): **B1** Pressable auf trust_score_card + weekly_challenge_highlight (Regel: InkWell nur Listen-Rows). **B2** Dashboard-Hero mit AnimatedEntrance(spring:true). **B4** NEU widgets/shared/count_up_text.dart (Zahlen zählen hoch, nur volle Ganzzahl-Strings, nur EffectsProfile.full) — in StatCard + Karma-Punkte. **B5** NEU widgets/shared/morphing_action_button.dart (Idle→Spinner→Häkchen in EINEM Button) — „Helfen" in post_detail morpht jetzt, Erfolgs-SnackBar entfällt (Feedback am Ort der Aktion), Fehler-SnackBar bleibt. **C1** map_screen: Post-Sheet offen → Karte AnimatedScale 0.985 (iOS-Modal-Tiefe, nur full). **D1** NEU widgets/effects/level_up_ceremony.dart (Karma-Levelwechsel-Zeremonie: easeOutBack-Enter, Stagger controller-frei, CelebrateBurst; Guard SecureStorage, erste Beobachtung setzt nur Basis) — Trigger in karma_widget. **D2** NEU widgets/shared/first_help_moment.dart (einmalige Dankes-Sequenz nach erster abgeschlossener Hilfe als Helfer:in: CinematicBackdrop-Header onboarding_connect + Teilen-CTA) — Hook in interactions_screen. **D3** NEU widgets/effects/living_flame.dart (2-Layer-CustomPainter-Flamme, full; reduced/none=statisches Icon) — im streak_widget. **E1** Mensa-Pulse: assistant FAB einmaliger Doppel-Puls (SecureStorage-Guard, nur full) — offener Punkt vom 06.06. erledigt. **E2** StatCard Skeleton→Wert via AnimatedSwitcher-Crossfade 150ms. **E3** EmptyStateCard moodAsset mit Ken-Burns-Mini-Drift (teilt Puls-Controller). i18n karma.levelUp.* + firstHelp.* in ALLEN 7 Sprachen. BEFUNDE: A2 (Hero PostCard→Detail) existiert bereits seit Phase 5.6; A3 (Wizard-SharedAxis) gegenstandslos — Create-Flow ist Single-Screen.
- [ ] Motion-Pakete Rest (braucht Version-Bump/Assets): A1 `animations`-Paket → AppTransitions.reveal (Container-Transform Karte→Detail); C4 Shader 2+3 (heat_shimmer/aurora — neue Assets erreichen Bestandsgeräte nicht via OTA-Patch); C5 OrbitViewer-Aktivierung (Turntable-Video generiert + assets/orbit/README.md-Pipeline; Frame-Extraktion braucht ffmpeg-Umgebung). [x] C3 SkyBody-Bogen ausgeliefert (4.4.1). B3 Pull-to-Refresh bewusst verworfen (Emil-Prinzip: häufige Aktion).
- [x] Design-/Motion-Skills global installiert (13 Stück): leonxlnx/taste-skill (taste/redesign/soft/minimalist/brutalist/brandkit/imagegen-web/imagegen-mobile/image-to-code/output/stitch), emilkowalski/skill (emil-design-eng) und pbakaus/impeccable (impeccable.style) — versioniert in `.agents/skills/` (SessionStart-Hook symlinkt nach `.claude/skills/` → in JEDER Session verfügbar), Einträge in skills-lock.json. Nutzungsregeln in CLAUDE.md verankert (Abschnitt „Design- & Motion-Skills"): emil-design-eng IMMER vor Animations-Arbeit, impeccable als UI-Default, taste-skill für Landing; Mensaena-Design-System hat bei Konflikten Vorrang.
- [x] Phase-5-Rest aufs EffectsGate: cinema_overlay nutzt jetzt effectsProfileProvider statt Einzel-Signale — damit greift ERSTMALS der Phase-6-Frame-Watchdog-Deckel auch auf den Cinema-Layer (full→reduced bei Jank ⇒ intensity 0.5 ⇒ teure Layer God-Rays/Flare/Fog/Dust/Grain fallen weg, Schwelle ≥0.6). intensityFactor (1.0/0.5/0.0) = exakt der alte CinemaIntensity.multiplier, verlustfrei. Lite-Mode bleibt bewusst STRENGER als das Gate (komplett aus — ARM32-Crash-Historie). film_grain braucht kein eigenes Gate (Leaf-Widget, wird von cinema_overlay gegated). AnimatedEntrance: neue `spring:`-Option (easeOutBack-Overshoot, bewusst controller-frei wegen V23-Crash-Historie) für Hero-Momente. Reine Dart-Fixes → OTA.
- [x] Motion-Design-Vorschlagspaket erstellt: docs/MOTION_DESIGN_VORSCHLAEGE.md — 5 Pakete (A räumliche Kontinuität/Container-Transform, B Mikro-Physik-Sweep, C Atmosphäre/Shader 2+3, D Stolz-Momente/Level-Up-Zeremonie, E geführte Wahrnehmung inkl. Mensa-Pulse) mit Reihenfolge-Empfehlung, OTA-vs-Bump-Kennzeichnung und EffectsGate-Leitplanken. Basis: emil-design-eng-Entscheidungsrahmen (Häufigkeit/Zweck/Easing/Dauer).
...
- [x] sb.from-Abbau Batch 10 (Posts+Marketplace-Widgets Sammel-PR): +6 Repo-Methoden — PostsRepository.similar/listSaveCollections/createSaveCollection, MarketplaceRepository.barterMatches/averagePrice, ConversationsRepository.hasMessages. 5 Widgets Supabase-frei (similar_posts_carousel/save_collection_picker/barter_matches_carousel/price_indicator/contact_requests_manager [Postcard via MessagesRepository.insertSystemNote]). Baseline: 53 Zugriffe / 33 Dateien (-53% seit Start 113/61).
- [x] sb.from-Abbau Batch 9 (Dashboard-Widgets Teil 2, komplex): +4 Repo-Methoden — DashboardWidgetsRepository.weeklyDigestCounts, ConversationsRepository.myMembershipsRead, MessagesRepository.recentFromOthers, ProfilesRepository.dashboardConfig. weekly_digest/unread_messages_widget/widget_grid_settings Supabase-frei (8 -> 0; unread nutzt zusaetzlich ProfilesRepository.getById fuer Sender). Baseline: 60 Zugriffe / 38 Dateien (-47% seit Start 113/61).
- [x] sb.from-Abbau Batch 8 (Dashboard-Widgets Sammel-PR, Teil 1): NEU repositories/dashboard_widgets_repository.dart (nearbyNewNeighbors/mentorEligibilityRow/pendingRatings[RPC+Fallback]/successStories[Bild-Fallback]/weeklyChallenges); 6 Widgets Supabase-frei (nearby_neighbors/become_mentor_cta/rating_prompt_banner/success_story_card/weekly_challenge_highlight + thanks_received via TrustRatingsRepository.getReceivedFor limit:5). Baseline: 68 Zugriffe / 41 Dateien. OFFEN (Teil 2): unread_messages_widget(3)/weekly_digest(3)/widget_grid_settings(2) — komplexe Multi-Query-Widgets.
...
- [x] sb.from-Abbau Batch 3 (Polls): NEU repositories/polls_repository.dart (fetch/vote/changeVote/create fuer post_polls + post_poll_votes); post_poll_widget (4 -> 0) + poll_create_sheet (1 -> 0) Supabase-frei, unused Import bereinigt. Baseline: 94 Zugriffe / 56 Dateien (Start: 113/61).
- [x] [mandatory] VERSION-BUMP 4.4.0+40400 — Cinematic-Bump-Paket: (1) video_player ^2.9.2 NEU + assets/videos/ deklariert; NEU widgets/effects/cinematic_video_backdrop.dart (Loop-Video NUR auf EffectsProfile.full, pausiert bei Watchdog-Downgrade, Init-Fehler/reduced/none -> frame-identisches Standbild via CinematicBackdrop; Scrim als CinematicBackdropScrim extrahiert -> Video+Bild dunkeln identisch ab); Landing nutzt jetzt das Splash-Loop-Video. (2) ERSTER GPU-SHADER shaders/grain.frag (pubspec shaders:-Sektion NEU) — film_grain rendert das Korn auf der GPU (0 Dart-Arbeit/Frame), CPU-Painter bleibt als Lade-/Fehler-Fallback. Bewusst NICHT im Bump: 3D-Viewer (keine GLB-Assets vorhanden), animations-Paket (reveal-Routen fehlen noch) — keine toten Dependencies. WICHTIG Release-Prozess: Minor-Bump -> shorebird_release.yml muss ggf. manuell dispatcht werden (siehe CLAUDE.md), flutter.yml registriert Release + app_releases (mandatory via Commit-Tag).
- [x] Design-System Phase 3 (Cinematic-Auftrag): NEU AppColors.sheetBackground — das Sheet-/Modal-Navy (0xF0121A28) war 58x als rohes Hex kopiert, jetzt EIN Token, Sweep ueber alle 57 Verwender (inkl. app_theme BottomSheetTheme). BEFUND korrigiert: AppSpacing existiert bereits seit 04.06. (Tailwind-paritaetische 4dp-Skala xxs-xl) — Audit-Aussage 'Spacing nicht tokenisiert' war falsch, offen ist nur die Adoption (inkrementell wie AppSnackBar). Damit ist das Design-System vollstaendig: Farben+sheetBackground, Typo, AppSpacing, AppMotion, AppLayout, Glas/Elevation-Bausteine.
- [x] Higgsfield-Asset-Paket Phase 1 (Cinematic-Medien): 5 Stills generiert (nano_banana_2, konsistenter Dusk-Diorama-Look passend zu AppColors) + komprimiert (WebP q80, 595 KB gesamt) in assets/images/: splash_dusk, onboarding_arrive, onboarding_connect, empty_nearby, empty_community. Splash-Loop-VIDEO (Seedance 2.0 image-to-video, Start=End-Frame=splash_dusk -> nahtloser Loop; ffmpeg H.264 1080x1920/24fps/2Mbps/stumm/faststart, 909 KB) liegt in assets/videos/ — bewusst NICHT im pubspec, Aktivierung erst mit dem video_player-Version-Bump. NEU widgets/effects/cinematic_backdrop.dart (EffectsGate: full=Ken-Burns-Drift 16s, reduced/none=statisch; Scrim oben/unten; errorBuilder->voidColor). Einbau: onboarding_tour (A3 Steps 0-2 / A4 ab 3, Crossfade + Step-Farb-Glow als Overlay), landing (A2 hinter Hero), EmptyStateCard.moodAsset NEU + verdrahtet in groups_screen (A6) und module_posts_screen (A5) — nur im wirklich-leer-Fall, nicht bei Filter-Treffern. Kosten: 46 Higgsfield-Credits.
...
- [x] Phase-6 Batch 2 — Grid-Sweep (kuratiert, 7 Grids): FixedCrossAxisCount -> MaxCrossAxisExtent mit Extents, die das Phone-Layout erhalten und Tablets automatisch mehr Spalten geben: badges (2->ext200), warnungen-Kacheln (2->ext240), profil-Badges (3->ext130), chat-Medien (3->ext130), knowledge-Kategorien (3->ext130), organizations-Kategorie-Sheet (3->ext130), event_create-Kategorien (5->ext96 = 4 Spalten Phone — fixt nebenbei den Phase-3-Befund 'Labels abgeschnitten'). NICHT angefasst: calendar (7=Wochentage, fachlich fix), admin-Grids (separater Batch), organization_suggest (DraggableSheet-Sonderfall).
- [x] Phase-6 Batch 1 — Layout-Tokens + Frame-Watchdog: NEU config/theme/app_layout.dart (Material-3-Fensterklassen compact/medium/expanded + context.layout-Extension + readableWidth/cardExtent/tileExtent-Tokens). NEU services/frame_watchdog_service.dart (SchedulerBinding.addTimingsCallback, Budget aus echter Display-Refresh-Rate +20% Toleranz, 120-Frame-Fenster, >10% Jank -> EffectsGate-Deckel full->reduced [nie none — nimmt Schmuck, keine Funktion], Recovery nach 60s stabil; Start/Stop in MensaenaApp wie MemoryWatchdog). effects_gate_provider: runtimeEffectsCapProvider eingebaut.
- [ ] Phase-6 Folge-Batches: Grid-Sweep crossAxisCount->maxCrossAxisExtent (30 Stellen, pro Grid Extent pruefen), Lesebreite ab medium (Create-Screens/Settings/Artikel), NavigationRail ab medium, Sheets->Dialoge, Master-Detail (3 Paare), HingeAware, Overflow-CI-Harness.
- [x] Phase-5b — Transition-Familie: page_transitions.dart zu AppTransitions umgebaut (forward=Standard-Alias mensaenaTransition [147 Aufrufstellen unveraendert], surface=Fade-Through, sheet=Slide-up mit gentle-Spring-Naeherung, immersive=Vollbild-Eintauchen, calm=160ms-Fade). JEDE Transition EffectsGate-integriert (none=instant, reduced=Fade, full=Choreografie) -> reduceMotion/seniorMode/Lite automatisch bedient. Router-Mapping: 5x calm (gesamter Krisen-Bereich), 16x sheet (alle Create-Flows), 3x immersive (Map/Call/LiveRoom). Nur Transform+Opacity (Impeller-freundlich, kein Blur mid-flight). reveal (Container-Transform) folgt mit dem animations-Paket im geplanten Version-Bump.
- [x] Phase-5 Schritt 4 — Pressable-Adoption Kern-Karten: post_card (Haupt-Tap, meistgenutzte Karte) + modules_hub _ModuleCard von InkWell auf Pressable (Spring-Scale + Haptik; doppeltes Haptics.tap entfernt). BEFUND korrigiert: AnimatedEntrance ist bereits in 15+ Listen-Screens adoptiert — Entrance-Sweep entfaellt (Audit-Stand veraltet). Regel bestaetigt: InkWell bleibt fuer Listen-Rows (Ripple), Pressable fuer Cards.
- [x] Phase-5 Schritt 3 — Gate-Migration + Spring-Feedback: tilt_card aufs EffectsGate (respektiert jetzt auch reduceMotion/Lite, vorher nur CinemaIntensity; unused cinema_theme-Import raus). glass_card: BackdropFilter NUR noch bei profile.full — auf reduced/none ersetzt eine staerker getoente Flaeche (alphaBlend auf surface 0.85) den Blur bei identischem Padding/Border (Blur war vorher IMMER an, auch Lite-Tier — teuerster Standard-Effekt). Pressable: AnimatedScale(110ms) -> physikbasiertes ScaleTransition mit AppMotion (snappy beim Druecken, bouncy-Release mit Mini-Overshoot), Controller sauber disposed.
- [x] Phase-5 Schritt 2 — EffectsGate + AppMotion: NEU providers/effects_gate_provider.dart (EffectsProfile none/reduced/full = Fusion aus effectiveReduceMotion [inkl. seniorMode] x Lite-Tier x CinemaIntensity; strengstes Signal gewinnt; intensityFactor-Helper; Phase-6-Frame-Watchdog dockt hier an). NEU config/motion/app_motion.dart (3 Spring-Tokens snappy/gentle/bouncy als SpringDescription + AnimationController.springTo()-Extension mit Velocity-Uebernahme — bewusst SDK-only, kein flutter_animate = kein Version-Bump). Erste Konsumenten: celebrate_burst + mini_confetti pruefen jetzt das Gate statt je EIN Einzelsignal (Confetti respektiert damit erstmals reduceMotion UND Lite UND Intensity zugleich).
- [x] Phase-5 Folge KOMPLETT: Effekt-Widgets aufs Gate (cinema_overlay 2026-06-12, tilt_card/glass_card-Blur Schritt 3, film_grain via cinema_overlay gegated), Pressable-Spring-Release via AppMotion (Schritt 3), AnimatedEntrance-Spring-Option (2026-06-12), Phase-5b-Transitions (done).
- [x] Phase-5-Fundament / M7 Effekt-Performance (Audit): atmospheric_layers — RepaintBoundary um alle 3 CustomPaints (Starfield/GodRays/Dust, 60fps-Repaints bleiben im eigenen Layer) + Dust-Painter Paint() aus der Partikel-Schleife gehoben. parallax_hero + parallax_image_header: didUpdateWidget tauscht Scroll-Listener beim Controller-Wechsel (vorher Doppel-Listener/toter Controller). tilt_card: Refcount-Unterlauf-Guard im geteilten Accelerometer-Stream. EINORDNUNG korrigiert: 'Offscreen-Ticker' (bloom/light_leaks) sind via SingleTickerProviderStateMixin+TickerMode auf inaktiven Routen bereits stumm — Rest (Scroll-Viewport) kommt mit dem EffectsGate in Phase 5.
- [x] Phase-3-UX Batch 3 (Kontrast + AppSnackBar-Migration Marktplatz): chat_message_bubble editedTag + posts_list Location-Hint von mute (#8B8576, ~4:1) auf inkSoft (WCAG-AA); Lese-Haekchen-Grau bewusst belassen (Zustands-Kontrast gelesen/ungelesen). AppSnackBar-Migration Modul Marktplatz: 10 handgebaute SnackBar-Bloecke in marketplace_detail (7) + marketplace_create (3) auf AppSnackBar.success/error/info — inkl. semantischer Korrektur (Fehler jetzt herzrotWarm statt neutral, Erfolg leben); der gestylte Barcode-Suchen-Toast (custom duration/elevated) bleibt bewusst manuell.
- [x] Phase-3-UX Batch 2 (Create-Flow + Robustheit): module_create_post_screen — textInputAction-Kette auf allen Kontaktfeldern (Telefon/E-Mail/WhatsApp/Treffpunkt next, Zeitbank done), Beschreibung maxLength 2000 + Counter, 3 weitere DE-Hardcodes auf .tr() (create.meetingPoint/timebankHours/timebankHoursHint NEU in 7 Sprachen, E-Mail-Label -> create.emailOptional). challenge_create + farm_create: autovalidateMode.onUserInteraction (Live-Validierung statt erst bei Submit). global_search: 8s-Timeout pro Suchquelle in safe() — eine haengende Query blockierte sonst das gesamte Future.wait (ewiger Spinner). BEFUNDE korrigiert: crisis_create-Counter existiert bereits (Flutter-Default bei maxLength), notifications_screen hat bereits error-Branch — keine Aenderung.
