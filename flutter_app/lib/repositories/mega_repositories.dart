...
class FollowsRepository {
  const FollowsRepository._();

  static Future<bool> isFollowing(String otherUserId) async {
  ...
  static Future<bool> follow(String otherUserId) async {
    final uid = SupabaseService.currentUser?.id;
    if (uid == null || uid == otherUserId) return false;
    try {
      await sb.from('user_follows').insert(
          {'follower_id': uid, 'following_id': otherUserId});
      return true;
    } catch (_) {
      return false;
    }
  ...
  static Future<bool> unfollow(String otherUserId) async {
    final uid = SupabaseService.currentUser?.id;
    if (uid == null) return false;
    try {
      await sb
  ...
  static Future<({int followers, int following})> counts(String userId) async {
    try {
      final f1 = await sb
          .from('user_follows')
          .select('id')
      ...
      return (followers: (f1 as List).length, following: (f2 as List).length);
    } catch (_) {
