...
CREATE POLICY post_comments_update ON post_comments FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY post_comments_delete ON post_comments FOR DELETE USING (auth.uid() = user_id OR auth.uid() = (SELECT p.user_id FROM posts p WHERE p.id = post_comments.post_id));

CREATE OR REPLACE FUNCTION update_post_comments_updated_at() RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
...
CREATE TRIGGER trg_post_comments_updated_at BEFORE UPDATE ON post_comments FOR EACH ROW EXECUTE FUNCTION update_post_comments_updated_at();
...
CREATE POLICY post_votes_update ON post_votes FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
SELECT post_id, coalesce(sum(vote), 0) AS score,
...
SELECT post_id, count(*) AS share_count FROM post_shares GROUP BY post_id;
