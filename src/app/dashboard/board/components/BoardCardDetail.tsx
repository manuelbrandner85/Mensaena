...
export default function BoardCardDetail({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div
        className={cn(
          'relative w-full sm:max-w-lg max-h-[90dvh] bg-mn-elevated rounded-t-2xl sm:rounded-2xl',
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {/* Full content */}
          <p className="text-sm text-mn-ink leading-relaxed whitespace-pre-wrap">{post.content}</p>
              ...
              <div className={`rounded-lg overflow-hidden grid gap-1 ${imgs.length > 1 ? 'grid-cols-2' : ''}`}>
                {imgs.map((src, i) => (
                  <img key={i} src={src} alt="" loading="lazy"
                    className="w-full max-h-56 object-contain bg-mn-elevated"
          ...
          {/* Contact box */}
          {post.contact_info && (
            <div className="bg-mn-surface rounded-lg p-3 border border-white/5">
              <p className="text-xs text-mn-mute mb-1">Kontaktinfo</p>
              <p className="text-sm text-mn-ink">{post.contact_info}</p>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              onClick={copyLink}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-mn-elevated text-mn-ink-soft hover:bg-mn-raised transition"
            >
          ...
          <div className="border-t border-white/5 pt-4">
            <h4 className="text-sm font-semibold text-mn-ink mb-3 flex items-center gap-1.5">
            ...
            {commentsLoading ? (
              <div className="flex items-center justify-center py-6">
                <Loader2 className="w-5 h-5 animate-spin text-mn-mute" />
              </div>
            ) : comments.length === 0 ? (
                  ...
                  return (
                    <div key={c.id} className="flex gap-2 group">
                      {c.profiles?.avatar_url ? (
                        ...
                        <div className="w-6 h-6 rounded-full bg-mn-raised flex items-center justify-center text-xs mt-0.5">
                          {cName.charAt(0).toUpperCase()}
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium text-mn-ink">{cName}</span>
                          <span className="text-xs text-mn-mute">{formatRelativeTime(c.created_at)}</span>
                          {isOwnComment && (
                            <button
                        ...
                        <p className="text-xs text-mn-ink-soft mt-0.5">{c.content}</p>
                      </div>
                    </div>
                  )
                })}
