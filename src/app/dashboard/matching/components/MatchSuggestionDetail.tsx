...
function PostSection({ post, label, colorClass }: { post: MatchPostSummary; label: string; colorClass: string }) {
  ...
  return (
    <div className={cn('rounded-xl p-4 border', colorClass)}>
      <p className="text-[10px] font-semibold uppercase tracking-wide mb-2 text-gray-500">{label}</p>
      <h4 className="text-sm font-semibold text-gray-900 mb-1">
        {typeConfig.emoji} {post.title}
      </h4>
      {post.description && (
        <p className="text-xs text-gray-600 mb-2 line-clamp-4">{post.description}</p>
      )}
      ...
      <div className="relative bg-white w-full max-w-lg max-h-[90vh] rounded-t-2xl sm:rounded-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-gray-900">Match-Details</h3>
            <span
              className={cn(
          ...
          {(match.status === 'suggested' || (match.status === 'pending' && !hasResponded)) && (
            <>
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-white text-gray-600 text-sm font-medium rounded-xl border border-gray-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors disabled:opacity-50"
              >
                {isResponding ? (
