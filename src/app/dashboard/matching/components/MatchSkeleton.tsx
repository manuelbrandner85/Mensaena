  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-8 w-24 bg-stone-200 rounded-full animate-pulse flex-shrink-0" />
        ))}
      </div>
      ...
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="bg-white rounded-xl border border-stone-100 p-4 animate-pulse"
        >
          <div className="flex items-start gap-3">
            {/* Score circle */}
            <div className="w-14 h-14 rounded-full bg-stone-200 flex-shrink-0" />
            {/* Content */}
