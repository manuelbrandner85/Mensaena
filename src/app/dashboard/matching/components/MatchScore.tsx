...
export default function MatchScore({
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {/* Label */}
      <span className={`font-medium text-mn-ink-soft ${cfg.label}`}>{label}</span>
          ...
          {SCORE_FACTORS.map((factor) => {
            ...
            return (
              <div key={factor.key} className="flex items-center gap-2">
                <span className="text-xs text-mn-mute w-20 truncate">{factor.label}</span>
                <div className="flex-1 h-1.5 bg-mn-elevated rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${pct}%`, backgroundColor: factor.color }}
                  />
                </div>
                <span className="text-xs text-mn-mute w-6 text-right">
                  {Math.round(value)}
                </span>
              </div>
            )
