  ...
  return (
    <>
      ...
      <div className="md:hidden overflow-x-auto -mx-4 px-4 pb-2">
        <div className="flex gap-1 min-w-max bg-stone-50 rounded-xl p-1">
          {TABS.map(tab => {
            const Icon = tab.icon
      ...
      <div className="hidden md:block w-[200px] flex-shrink-0">
        <nav className="relative bg-white rounded-2xl shadow-soft border border-stone-100 p-2 sticky top-24 overflow-hidden">
          <div
          ...
          <div className="relative pt-1">
            {TABS.map(tab => {
              ...
              return (
                ...
                >
                  <Icon className={cn('w-4 h-4 transition-transform', active ? 'text-primary-600 scale-110' : 'text-ink-400')} />
                  {t(tab.id)}
                  {dirty && (
