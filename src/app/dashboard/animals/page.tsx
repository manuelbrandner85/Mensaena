...
function AnimalStatusWidget() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Letzte Notfälle */}
      {recentLost.length > 0 && (
        <div className="relative bg-gradient-to-br from-red-50 via-red-50/80 to-orange-50 border border-red-200 rounded-2xl p-4 shadow-cinema-card overflow-hidden">
          <div
          ...
          <div className="relative space-y-1.5">
            {recentLost.map(p => (
              <Link key={p.id} href={`/dashboard/posts/${p.id}`}
                className="flex items-center gap-2 p-2.5 bg-mn-elevated rounded-xl hover:bg-red-50 transition-all border border-red-100 group shadow-cinema-card">
                <PawPrint className="w-4 h-4 text-red-400 flex-shrink-0 group-hover:scale-110 transition-transform" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-mn-ink truncate group-hover:text-red-700">{p.title}</p>
                  {p.location_text && (
