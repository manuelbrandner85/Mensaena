...
export default function BotTipCard({ tipText }: BotTipCardProps) {
  ...
  return (
    <Card
      variant="flat"
      padding="md"
      className="bg-gradient-to-br from-mn-bronze/8 to-primary-100/50 border-mn-bronze/20/50"
    >
      {/* Header */}
      <div className="flex items-center gap-2">
        ...
        <Lightbulb className="w-4 h-4 text-mn-bronze ml-auto" />
      </div>
      ...
      <div className="flex justify-between items-center mt-3">
        <Link
          href="/dashboard/chat"
          className="text-xs text-mn-bronze hover:text-primary-900 font-medium transition-colors"
        >
          Chat mit Bot →
        </Link>
        <button
          onClick={handleNextTip}
          className="flex items-center gap-1 text-xs text-mn-bronze hover:text-primary-800 transition-colors"
        >
