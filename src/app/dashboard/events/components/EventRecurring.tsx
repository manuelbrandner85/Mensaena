    ...
    <div className="space-y-3">
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        ...
        >
          <span
            className={cn(
              'absolute top-0.5 w-4 h-4 bg-mn-elevated rounded-full shadow transition-transform',
              enabled ? 'translate-x-5' : 'translate-x-0.5',
            )}
          />
        </button>
        <div className="flex items-center gap-1.5">
          <Repeat className="w-4 h-4 text-mn-ink-soft" />
          <span className="text-sm font-medium text-mn-ink-soft">Wiederkehrende Veranstaltung</span>
        </div>
      </label>
      ...
      {enabled && (
        <div className="pl-[52px] space-y-3">
          <div>
            <label className="text-sm text-mn-ink-soft mb-1 block">Wiederholung</label>
            <select
              value={frequency}
          ...
          <div>
            <label className="text-sm text-mn-ink-soft mb-1 block">Wiederholen bis</label>
            <input
              type="date"
              value={until}
