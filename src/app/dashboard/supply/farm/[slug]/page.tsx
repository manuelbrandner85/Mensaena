...
function SimilarFarms({ farm }: { farm: FarmListing }) {
  ...
  if (loading) return (
    <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
      <h2 className="font-bold text-mn-ink mb-4">Ähnliche Betriebe</h2>
      <div className="grid grid-cols-2 gap-3">
        {[1,2,3,4].map((i) => <div key={i} className="bg-mn-elevated rounded-xl h-24 animate-pulse" />)}
      </div>
    </div>
  )
  ...
  return (
    <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
      <h2 className="flex items-center gap-2 font-bold text-mn-ink mb-4">
...
function SharePanel({ farm }: { farm: FarmListing }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-50 bg-mn-elevated rounded-2xl shadow-xl border border-white/5 p-4 w-64">
          <p className="text-xs font-semibold text-mn-mute uppercase tracking-wide mb-3">Teilen über</p>
          <div className="space-y-2">
            ...
            >
              <Copy className="w-4 h-4 text-mn-mute" />
...
export default function FarmDetailPage() {
    ...
    return (
      <div className="min-h-dvh flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-mn-bronze/30 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
    ...
    return (
      <div className="min-h-dvh flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-mn-ink mb-2">Betrieb nicht gefunden</h2>
  ...
  return (
    <div className="min-h-dvh bg-mn-void">
      {/* Back bar */}
      <div className="bg-mn-elevated border-b border-white/5 px-4 sm:px-6 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-sm text-mn-ink-soft hover:text-mn-ink font-medium transition-colors"
          >
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="bg-mn-elevated rounded-3xl shadow-sm border border-white/5 overflow-hidden">
          <div className="bg-gradient-to-r from-mn-bronze/12 to-mn-bronze-warm/8 h-3" />
          <div className="p-6 md:p-8">
            <div className="flex items-start gap-5">
              <div className="w-16 h-16 bg-gradient-to-br from-mn-bronze/12 to-yellow-100 rounded-2xl flex items-center justify-center text-3xl shrink-0 border border-amber-200">
                {categoryIcon}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  {farm.is_seasonal && (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-mn-elevated text-mn-bronze-warm">
                      🍂 Saisonal
                    </span>
                  )}
                </div>
                <h1 className="text-2xl md:text-3xl font-bold text-mn-ink mb-1">{farm.name}</h1>
                <p className="flex items-center gap-1.5 text-mn-mute text-sm flex-wrap">
            ...
            {farm.description && (
              <p className="mt-5 text-mn-ink-soft leading-relaxed text-base">{farm.description}</p>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {farm.delivery_options.length > 0 && (
              <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
                <h2 className="flex items-center gap-2 text-lg font-bold text-mn-ink mb-4">
                  <Truck className="w-5 h-5 text-mn-bronze" /> Liefer- & Abholoptionen
                </h2>
                <div className="space-y-2">
                  {farm.delivery_options.map((d) => (
                    <div key={d} className="flex items-center gap-3 text-sm text-mn-ink-soft">
            ...
            {farm.subcategories.length > 0 && (
              <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
                <h2 className="flex items-center gap-2 text-lg font-bold text-mn-ink mb-4">
                  <Building2 className="w-5 h-5 text-mn-mute" /> Kategorien & Schwerpunkte
                </h2>
                <div className="flex flex-wrap gap-2">
                  {farm.subcategories.map((s) => (
                    <span key={s} className="bg-mn-elevated text-mn-ink-soft text-sm px-3 py-1.5 rounded-full">{s}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-5">
              <h2 className="font-bold text-mn-ink mb-4 text-base">Kontakt & Adresse</h2>
              <div className="space-y-3 text-sm">
                {farm.address && (
                  <div className="flex items-start gap-2.5 text-mn-ink-soft">
                    <MapPin className="w-4 h-4 mt-0.5 text-mn-mute shrink-0" />
                    <div>
                      <p>{farm.address}</p>
            ...
            {farm.opening_hours && Object.keys(farm.opening_hours).length > 0 && (
              <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-5">
                <h2 className="flex items-center gap-2 font-bold text-mn-ink mb-4 text-base">
                  <Clock className="w-4 h-4 text-mn-mute" /> Öffnungszeiten
                </h2>
                <div className="space-y-1.5">
                  {Object.entries(farm.opening_hours).map(([day, hours]) => (
                    <div key={day} className="flex items-center justify-between text-sm">
                      <span className="text-mn-mute">{WEEKDAYS_DE[day.toLowerCase()] ?? day}</span>
                      <span className="text-mn-ink font-medium">{String(hours)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Datenquelle */}
            {farm.source_name && (
              <div className="bg-mn-surface rounded-2xl border border-white/5 p-4 text-xs text-mn-mute">
