      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6 space-y-4">
            <h2 className="font-bold text-mn-ink text-lg">Grunddaten</h2>

            <div>
              <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Betriebsname *</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Betriebstyp *</label>
              <select
                value={form.category}
            ...
            <div>
              <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Beschreibung</label>
              <textarea
                value={form.description}
                onChange={(e) => set('description', e.target.value)}
                rows={3}
                placeholder="Kurze Beschreibung deines Betriebs, was du anbietest, was dich besonders macht…"
                className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300 resize-none"
              />
            </div>
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-mn-ink-soft flex items-center gap-1"><Leaf className="w-3.5 h-3.5 text-lime-600" /> Bio-zertifiziert</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-mn-ink-soft">🍂 Saisonal</span>
              </label>
            </div>
          </div>
          ...
          <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {previews.map((src, idx) => (
                <div key={idx} className="relative">
                  <img
                    src={src}
                    alt=""
                    className="h-24 w-24 object-cover rounded-xl border border-white/5"
              ...
              {photoUploading && (
                <div className="h-24 w-24 rounded-xl border border-dashed border-white/8 flex items-center justify-center bg-mn-surface">
              ...
              {previews.length < MAX_PHOTOS && !photoUploading && (
                <button
                  ...
                  className="h-24 w-24 inline-flex flex-col items-center justify-center gap-1.5 text-xs text-mn-mute border border-dashed border-white/8 rounded-xl hover:bg-mn-surface transition"
                >
            ...
            <p className="text-xs text-mn-mute mt-2">Bis zu {MAX_PHOTOS} Fotos · max. 8 MB je Bild · JPEG, PNG oder WebP</p>
          </div>

          {/* Adresse */}
          <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6 space-y-4">
            <h2 className="font-bold text-mn-ink text-lg">Adresse</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Straße & Hausnummer</label>
                <input type="text" value={form.address} onChange={(e) => set('address', e.target.value)}
                  placeholder="Musterstraße 1" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div>
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Postleitzahl</label>
                <input type="text" value={form.postal_code} onChange={(e) => set('postal_code', e.target.value)}
                  placeholder="1010" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div>
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Ort *</label>
                <input type="text" value={form.city} onChange={(e) => set('city', e.target.value)}
                  placeholder="Wien" required className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div>
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Bundesland / Kanton</label>
                <input type="text" value={form.state} onChange={(e) => set('state', e.target.value)}
                  placeholder="Niederösterreich" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div>
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Land</label>
          ...
          <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Telefon</label>
                <input type="tel" value={form.phone} onChange={(e) => set('phone', e.target.value)}
                  placeholder="+43 1 234567" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div>
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">E-Mail</label>
                <input type="email" value={form.email} onChange={(e) => set('email', e.target.value)}
                  placeholder="hof@beispiel.at" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div className="sm:col-span-2">
                <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Website</label>
                <input type="url" value={form.website} onChange={(e) => set('website', e.target.value)}
                  placeholder="https://www.meinbauernhof.at" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
            </div>
          </div>
          ...
          {/* Produkte */}
          <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
            <h2 className="font-bold text-mn-ink text-lg mb-4">Produkte</h2>
            <div className="flex flex-wrap gap-2">
              {FARM_PRODUCTS.map((p) => (
                <button
                  type="button"
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm border font-medium transition-all ${
                    form.products.includes(p)
                      ...
                      : 'bg-mn-surface text-mn-ink-soft border-white/5 hover:border-amber-300'
                  }`}
                >
                  {p}
                </button>
          ...
          <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={form.services} onChange={(e) => set('services', e.target.value)}
                placeholder="Hofladen, Führungen, Käserei, Selbsternte" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
            </div>
            <div>
              <label className="text-sm font-medium text-mn-ink-soft mb-1.5 block">Liefer-/Abholoptionen (kommagetrennt)</label>
              <input type="text" value={form.delivery_options} onChange={(e) => set('delivery_options', e.target.value)}
                placeholder="Abholung Hof, Lieferung Wien, Versand Österreich" className="w-full border border-white/5 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <Link href="/dashboard/supply"
              className="flex-1 py-3 border border-white/5 rounded-xl text-sm font-semibold text-mn-ink-soft hover:bg-mn-surface transition-colors text-center">
              Abbrechen
            </Link>
            <button
              type="submit"
