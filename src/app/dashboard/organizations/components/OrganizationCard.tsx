    ...
    >
      ...
      <div className="p-4 pt-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {org.distance_km != null && (
                <span className="text-xs text-mn-mute">{org.distance_km} km</span>
              )}
            </div>
        ...
        {(org.short_description || org.description) && (
          <p className="text-xs text-mn-ink-soft mt-2 leading-relaxed line-clamp-2">
            {org.short_description || org.description}
          </p>
        )}
        ...
        {org.tags && org.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {org.tags.slice(0, 4).map(tag => (
              <span key={tag} className="text-xs bg-mn-surface text-mn-mute px-1.5 py-0.5 rounded-full">
                {tag}
              </span>
            ))}
            {org.tags.length > 4 && (
              <span className="text-xs text-mn-mute">+{org.tags.length - 4}</span>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mt-3 flex-wrap">
          ...
          {org.website && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1 text-xs bg-mn-surface text-mn-ink-soft hover:bg-mn-elevated px-2.5 py-1 rounded-full transition-colors"
            >
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs text-mn-mute hover:text-mn-ink-soft px-2 py-1 rounded-full transition-colors"
      ...
      {/* Expanded details */}
      {expanded && (
        <div className="border-t border-white/5 bg-mn-surface/50 px-4 py-3 space-y-2">
          {(org.address || org.city) && (
            <div className="flex items-start gap-2 text-xs text-mn-ink-soft">
              <MapPin className="w-3.5 h-3.5 mt-0.5 text-mn-mute flex-shrink-0" />
          ...
          {(org.opening_hours_text || org.opening_hours?.notes) && (
            <div className="flex items-start gap-2 text-xs text-mn-ink-soft">
              <Clock className="w-3.5 h-3.5 mt-0.5 text-mn-mute flex-shrink-0" />
          ...
          {org.services && org.services.length > 0 && (
            <div className="flex flex-wrap gap-1 pt-1">
              {org.services.map(s => (
                <span key={s} className="text-xs bg-mn-elevated border border-white/5 text-mn-ink-soft px-2 py-0.5 rounded-full">
                  {s}
                </span>
              ))}
            </div>
