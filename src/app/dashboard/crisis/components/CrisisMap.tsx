...
export default function CrisisMap({ crises, loading, className, height = 'h-[400px]', onCrisisClick }: Props) {
  ...
  return (
    <div className={cn('relative rounded-2xl overflow-hidden border border-white/5', height, className)}>
      {(loading || !mapReady) && (
        <div className="absolute inset-0 bg-mn-elevated flex items-center justify-center z-20">
          <div className="text-center">
            <Loader2 className="w-6 h-6 text-mn-mute animate-spin mx-auto mb-2" />
            <p className="text-xs text-mn-mute">Karte wird geladen...</p>
          </div>
        </div>
      )}
