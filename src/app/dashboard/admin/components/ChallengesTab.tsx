  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <>
          <p className="text-sm text-mn-mute">{total} Challenges gesamt</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-mn-surface border-b border-white/5">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-mn-ink-soft">Challenge</th>
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-mn-ink-soft">Endet</th>
                ...
                <tbody className="divide-y divide-stone-100">
                  {challenges.map(c => (
                    <Fragment key={c.id}>
                      <tr className="hover:bg-mn-surface transition-colors">
                        <td className="px-4 py-3">
                          <p className="font-medium text-mn-ink">{c.title}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className={`inline-block px-1.5 py-0.5 rounded-full text-xs font-semibold ${DIFFICULTY_COLORS[c.difficulty] ?? 'bg-mn-elevated text-mn-ink-soft'}`}>
                              {c.difficulty}
                            </span>
                            <span className="text-xs text-amber-600 font-medium">{c.points} Pkt.</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-mn-ink-soft text-xs">{CATEGORY_LABELS[c.category] ?? c.category}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLORS[c.status] ?? 'bg-mn-elevated text-mn-ink-soft'}`}>
                      ...
                      {expanded === c.id && (
                        <tr className="bg-mn-surface">
                          <td colSpan={6} className="px-6 py-3">
                            <p className="text-xs font-semibold text-mn-mute mb-2">Teilnehmer</p>
                            {!participants[c.id] ? (
                              ...
                              <p className="text-xs text-mn-mute">Keine Teilnehmer</p>
                            ) : (
                              <div className="flex flex-wrap gap-2">
                                {participants[c.id].map(p => (
                                  <div key={p.id} className="flex items-center gap-1.5 bg-mn-elevated border border-white/5 rounded-lg px-2.5 py-1 text-xs">
                  ...
                  {challenges.length === 0 && (
                    <tr><td colSpan={6} className="px-4 py-12 text-center text-mn-mute">Keine Challenges gefunden</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-mn-elevated rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-mn-mute mb-1">Beschreibung</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-mn-mute mb-1">Schwierigkeit</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-mn-mute mb-1">Punkte</label>
                  <input type="number" value={newPoints} onChange={e => setNewPoints(Number(e.target.value))} min={1}
                    className="w-full px-3 py-2 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-mn-mute mb-1">Enddatum *</label>
