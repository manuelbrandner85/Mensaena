  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm overflow-hidden">
        <div className="border-b border-white/5 px-4 py-3 flex items-center gap-2 overflow-x-auto">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-mn-amber/5 border border-primary-100 rounded-2xl p-4">
        ...
        <p className="text-xs text-mn-ink-soft mb-3">Erstellt automatisch einen Newsletter aus der Plattform-Aktivität der letzten 7 Tage (neue Beiträge, Events, Gruppen, Mitglieder).</p>
        <button
      ...
      <div className="bg-mn-elevated border border-white/5 rounded-2xl p-4 shadow-sm">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-lg">🤖</span>
          <h3 className="text-sm font-bold text-mn-ink">Newsletter zu beliebigem Thema</h3>
        </div>
        <p className="text-xs text-mn-ink-soft mb-3">KI generiert einen professionellen Newsletter zu jedem gewünschten Thema — Sommer-Aktionen, Sicherheitstipps, Feiertage, uvm.</p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 bg-mn-elevated border border-white/5 hover:bg-mn-surface text-mn-ink-soft rounded-xl text-sm font-medium transition-colors"
        >
        ...
        <button
          onClick={load}
          className="inline-flex items-center gap-2 px-3 py-2 text-mn-mute hover:bg-mn-elevated rounded-xl text-sm transition-colors"
        >
      ...
      {/* Standard-Templates */}
      <div className="bg-mn-elevated border border-white/5 rounded-2xl p-4">
        <p className="text-xs font-bold text-mn-ink-soft mb-3">Standard-Vorlagen (klicken zum Erstellen):</p>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-5 h-5 text-mn-ghost animate-spin" />
        </div>
      ) : (
        <>
          {/* Entwürfe */}
          <section>
            ...
            {drafts.length === 0 ? (
              <div className="text-sm text-mn-mute italic bg-mn-surface rounded-xl p-6 text-center">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {socialResult.hashtags.map((tag, i) => (
                    <span key={i} className="px-2 py-1 bg-mn-elevated text-mn-ink-soft rounded-lg text-xs font-medium">{tag}</span>
                  ))}
                </div>
                <p className="text-xs text-mn-mute">Posts wurden als Entwurf in Social Media gespeichert. Bearbeiten & veröffentlichen unter Marketing → Social Media.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {sendCampaignId && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <div className="bg-mn-elevated rounded-2xl shadow-2xl w-full max-w-lg max-h-[90dvh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              <h3 className="text-sm font-bold text-mn-ink">Kampagne versenden</h3>
              <button onClick={() => setSendCampaignId(null)} className="text-mn-mute hover:text-mn-ink text-xl">×</button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div className="bg-gradient-to-r from-primary-50 to-mn-teal-soft border border-primary-100 rounded-xl p-4">
                ...
                {aiScore && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-2 bg-mn-raised rounded-full overflow-hidden">
                        <div
                      ...
                      <span className="text-sm font-bold text-mn-ink tabular-nums">{aiScore.score}%</span>
                    </div>
                    <p className="text-xs text-mn-ink-soft">{aiScore.feedback}</p>
                    {aiScore.alternatives.length > 0 && (
                      <div className="space-y-1">
                        <p className="text-xs font-bold text-mn-mute uppercase tracking-wide">Bessere Alternativen:</p>
                ...
                {!aiScore && !aiScoring && (
                  <p className="text-xs text-mn-mute">Analysiert Betreffzeile + gibt Verbesserungsvorschläge basierend auf bisherigen Öffnungsraten.</p>
                )}
              </div>

              {/* Multi-Channel */}
              <div className="border border-white/5 rounded-xl p-4">
                <p className="text-xs font-bold text-mn-ink-soft mb-3">Kanäle wählen</p>
                <div className="flex flex-wrap gap-2">
                  {[
              ...
              {sendMode === 'segment' && (
                <div className="space-y-2">
                  <label className="block text-xs font-bold text-mn-ink-soft mb-1">Empfänger-Gruppe wählen</label>
                  <select
                    value={sendSegment}
              ...
              {sendMode === 'specific' && (
                <div>
                  <label className="block text-xs font-bold text-mn-ink-soft mb-1">E-Mail-Adressen (je Zeile eine oder kommagetrennt)</label>
                  <textarea
                    value={sendEmails}
            ...
            <div className="p-5 border-t border-white/5 flex justify-end gap-2">
              <button onClick={() => setSendCampaignId(null)} className="px-4 py-2 text-mn-ink-soft hover:bg-mn-elevated rounded-xl text-sm">Abbrechen</button>
              <button
                onClick={doSend}
...
function CampaignEditModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="bg-mn-elevated rounded-2xl shadow-2xl w-full max-w-lg sm:max-w-2xl lg:max-w-4xl max-h-[90dvh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <h3 className="text-lg font-bold text-mn-ink">
            {isNew ? 'Neue Kampagne' : 'Kampagne bearbeiten'}
          </h3>
          <button onClick={onClose} className="text-mn-mute hover:text-mn-ink text-2xl leading-none" aria-label="Schließen">×</button>
        </div>

        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div>
            <label className="block text-xs font-bold text-mn-ink-soft mb-1.5">Betreff</label>
            <input
              type="text"
              value={subject}
          ...
          <div>
            ...
            {showPreview ? (
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-white/5 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-primary-400"
                rows={18}
        ...
        <div className="p-5 border-t border-white/5 flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2 text-mn-ink-soft hover:bg-mn-elevated rounded-xl text-sm font-medium"
          >
            Abbrechen
          </button>
          <button
...
function PreviewModal({ campaign, onClose }: { campaign: AdminEmailCampaign; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="bg-mn-elevated rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90dvh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <div className="min-w-0">
            <h3 className="text-lg font-bold text-mn-ink truncate">{campaign.subject}</h3>
            <p className="text-xs text-mn-mute mt-0.5">Vorschau</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 bg-mn-elevated rounded-xl p-1">
              <button
                onClick={() => setDevice('desktop')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${device === 'desktop' ? 'bg-mn-elevated shadow-sm text-mn-ink' : 'text-mn-mute hover:text-mn-ink-soft'}`}
              >
                <Monitor className="w-3.5 h-3.5" /> Desktop
              </button>
              <button
                onClick={() => setDevice('mobile')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${device === 'mobile' ? 'bg-mn-elevated shadow-sm text-mn-ink' : 'text-mn-mute hover:text-mn-ink-soft'}`}
              >
                <Smartphone className="w-3.5 h-3.5" /> Mobile
              </button>
            </div>
            <button onClick={onClose} className="text-mn-mute hover:text-mn-ink text-2xl leading-none">×</button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto bg-mn-elevated flex justify-center p-4">
          <div style={{ width: device === 'mobile' ? '375px' : '100%' }} className="transition-all duration-300">
            <iframe srcDoc={campaign.html_content} className="w-full bg-mn-elevated rounded-xl shadow-sm" style={{ minHeight: '600px', border: 'none' }} />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-mn-amber/5 border border-primary-100 rounded-2xl p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-mn-ink-soft mt-1 leading-relaxed">
      ...
      <div className="bg-mn-elevated border border-white/5 rounded-2xl p-5">
        ...
        {loadingLogs ? (
          <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 text-mn-ghost animate-spin" /></div>
        ) : logs.length === 0 ? (
          <p className="text-xs text-mn-mute italic text-center py-4">Noch keine Willkommensmails versendet.</p>
        ) : (
          <div className="divide-y divide-stone-100 max-h-64 overflow-y-auto">
            {logs.map(log => (
              <div key={log.id} className="flex items-center justify-between py-2">
                ...
                <span className="text-xs text-mn-mute flex-shrink-0 ml-2">
      ...
      {/* Vorschau */}
      <div className="bg-mn-elevated border border-white/5 rounded-2xl p-5">
        <h4 className="text-sm font-bold text-mn-ink mb-3">Vorschau</h4>
        <div className="border border-white/5 rounded-xl overflow-hidden bg-mn-surface">
          <iframe
            src="/api/emails/welcome-preview"
            className="w-full h-[600px] bg-mn-elevated"
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-mn-mute" />
          <input
            type="text"
            inputMode="search"
            value={query}
            ...
            className="w-full pl-9 pr-3 py-2 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400"
          />
        </div>
        <select
          value={filter}
          ...
          className="px-3 py-2 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400"
        >
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs font-bold text-mn-mute uppercase tracking-wider border-b border-white/5">
            ...
            <tbody className="divide-y divide-stone-100">
              {filtered.map(s => (
                <tr key={s.id} className="hover:bg-mn-surface">
                  <td className="px-3 py-2.5 text-mn-ink">{s.email}</td>
                  ...
                  <td className="px-3 py-2.5">
                    {s.subscribed ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-mn-elevated text-mn-ink-soft rounded-full text-xs font-medium">
                        <XCircle className="w-3 h-3" /> Abgemeldet
                      </span>
                    )}
                  </td>
                  <td className="px-3 py-2.5 text-mn-mute text-xs">
...
function ComplianceView() {
  ...
  if (loading) return <div className="flex items-center justify-center py-16"><Loader2 className="w-5 h-5 animate-spin text-mn-ghost" /></div>
  if (!data) return null
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        {metrics.map(m => (
          <div key={m.label} className="bg-mn-elevated border border-white/5 rounded-2xl p-4 shadow-sm">
            ...
            <p className="text-xs font-semibold text-mn-ink-soft mt-0.5">{m.label}</p>
            <p className="text-xs text-mn-mute mt-0.5">{m.sub}</p>
            {m.status === 'warn' && (
      ...
      <div className="bg-mn-elevated border border-white/5 rounded-2xl p-5">
        <h4 className="text-sm font-bold text-mn-ink mb-3">DSGVO-Checkliste</h4>
        <div className="space-y-2">
          {[
...
function DripView() {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12"><Loader2 className="w-5 h-5 animate-spin text-mn-ghost" /></div>
      ) : campaigns.length === 0 ? (
        <div className="bg-mn-surface rounded-2xl p-8 text-center text-sm text-mn-mute">
            ...
            <div key={drip.id} className="flex items-center gap-3 p-4 bg-mn-elevated border border-white/5 hover:border-mn-amber/20 rounded-xl transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="w-9 h-9 flex items-center justify-center rounded-lg text-mn-mute hover:bg-mn-elevated"
                >
                  <Edit3 className="w-4 h-4" />
                </button>
              </div>
  ...
  return (
    <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-sm font-bold text-mn-ink">Re-Engagement Automation</h3>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <label className="text-xs text-mn-ink-soft whitespace-nowrap">Inaktiv seit:</label>
          <select
            value={inactiveDays}
            onChange={e => setInactiveDays(Number(e.target.value))}
            aria-label="Inaktivität in Tagen"
            className="px-2 py-1.5 border border-amber-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-amber-400 bg-mn-elevated"
          >
      ...
      {result && (
        <div className="mt-3 text-xs text-mn-ink-soft bg-mn-elevated rounded-lg px-3 py-2 border border-amber-100">
          {result.enrolled > 0
...
function DripEditModal({ drip, onClose, onSaved }: { drip: DripCampaign; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="bg-mn-elevated rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90dvh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <h3 className="text-sm font-bold text-mn-ink">{isNew ? 'Neuer Drip-Funnel' : 'Funnel bearbeiten'}</h3>
          <button onClick={onClose} className="text-mn-mute hover:text-mn-ink text-2xl">×</button>
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-mn-ink-soft mb-1.5">Name</label>
              <input value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" placeholder="Willkommens-Funnel" />
            </div>
            <div>
              <label className="block text-xs font-bold text-mn-ink-soft mb-1.5">Trigger</label>
              <select value={triggerType} onChange={e => setTriggerType(e.target.value)} aria-label="Trigger-Typ" className="w-full px-3 py-2 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
          ...
          <div>
            <label className="block text-xs font-bold text-mn-ink-soft mb-1.5">Beschreibung (optional)</label>
            <input value={description} onChange={e => setDescription(e.target.value)} className="w-full px-3 py-2 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" placeholder="Kurze Beschreibung" />
          </div>
          ...
          <div className="bg-gradient-to-r from-mn-amber to-mn-teal-soft border border-white/5 rounded-xl p-4">
            ...
            <p className="text-xs text-mn-mute mt-1.5">KI erstellt 5 professionelle E-Mails passend zum gewählten Trigger. Danach manuell editierbar.</p>
          </div>
            ...
            <div className="space-y-3">
              {steps.map((step, i) => (
                <div key={i} className="border border-white/5 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-24">
                      <label className="block text-xs text-mn-mute mb-1">Verzögerung (Tage)</label>
                      <input type="number" min={0} value={step.delay_days} onChange={e => setSteps(s => s.map((x, j) => j === i ? { ...x, delay_days: parseInt(e.target.value) || 0 } : x))} className="w-full px-2 py-1.5 border border-white/5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
                    </div>
                    <div className="flex-1">
                      <label className="block text-xs text-mn-mute mb-1">Betreff</label>
                      <input value={step.subject} onChange={e => setSteps(s => s.map((x, j) => j === i ? { ...x, subject: e.target.value } : x))} className="w-full px-2 py-1.5 border border-white/5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" placeholder="Hallo {{vorname}}, …" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs text-mn-mute mb-1">HTML-Inhalt</label>
                    <textarea value={step.html_content} onChange={e => setSteps(s => s.map((x, j) => j === i ? { ...x, html_content: e.target.value } : x))} className="w-full px-2 py-1.5 border border-white/5 rounded-lg text-xs font-mono focus:outline-none focus:ring-2 focus:ring-primary-400" rows={4} placeholder="<p>Hallo {{vorname}}, …</p>" />
                  </div>
                </div>
              ))}
            </div>
