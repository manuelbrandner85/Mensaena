...
export default function OverviewTab({ stats }: Props) {
  ...
  const cards: { icon: React.ReactNode; label: string; value: number | string; bg: string; sub?: string }[] = [
    ...
    { icon: <FileText className="w-5 h-5 text-emerald-600" />, label: 'Beitraege gesamt', value: stats.total_posts ?? 0, bg: 'bg-emerald-50', sub: `${stats.active_posts ?? 0} aktiv` },
    { icon: <Activity className="w-5 h-5 text-amber-600" />, label: 'Neue Beitraege (7d)', value: stats.new_posts_7d ?? 0, bg: 'bg-amber-50' },
    ...
    { icon: <LayoutGrid className="w-5 h-5 text-orange-600" />, label: 'Brett-Beitraege', value: stats.total_board_posts ?? 0, bg: 'bg-orange-50', sub: `${stats.active_board_posts ?? 0} aktiv` },
