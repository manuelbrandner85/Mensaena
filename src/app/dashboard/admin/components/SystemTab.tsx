      ...
      <div className="bg-mn-elevated rounded-2xl border border-white/5 shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-mn-mute mt-1">
        ...
        {lastRun && (
          <p className="text-xs text-mn-mute flex items-center gap-1 mb-4">
        ...
        {cleanupResult && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {Object.entries(cleanupResult).map(([key, value]) => (
              <div key={key} className="bg-mn-surface rounded-xl p-4">
                <p className="text-xs text-mn-mute mb-1 capitalize">{key.replace(/_/g, ' ')}</p>
                <p className="text-xl font-bold text-mn-ink">{String(value)}</p>
              </div>
            ))}
          </div>
        )}
          ...
          {logs.map(log => (
            <div key={log.id} className="flex items-start gap-3 px-3 py-2.5 bg-mn-surface rounded-xl text-xs">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-mn-mute">
                  von {log.profiles?.name ?? 'System'}
                  {log.target_type && ` • ${log.target_type}`}
                </p>
                {log.details && Object.keys(log.details).length > 0 && (
                  <p className="text-mn-mute truncate mt-0.5">
                    {JSON.stringify(log.details).slice(0, 80)}
                  </p>
                )}
              </div>
              <span className="text-mn-mute whitespace-nowrap">
...
function InfoCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 bg-mn-surface rounded-xl">
      {icon}
      <div>
        <p className="text-xs text-mn-mute">{label}</p>
        <p className="text-sm font-medium text-mn-ink">{value}</p>
      </div>
    </div>
  )
}
