      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <div className="bg-mn-surface rounded-xl p-4 border border-white/5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-medium text-mn-mute">Einträge gesamt</span>
          </div>
          <p className="text-2xl font-bold text-mn-ink">{total}</p>
        </div>
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-mn-surface border-b border-white/5">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-mn-ink-soft">Geber → Empfänger</th>
                    <th className="text-left px-4 py-3 font-semibold text-mn-ink-soft">Beschreibung</th>
                    <th className="text-center px-4 py-3 font-semibold text-mn-ink-soft">Stunden</th>
                ...
                <tbody className="divide-y divide-stone-100">
                  {entries.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="hover:bg-mn-surface transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5 text-xs">
                            <span className="font-medium text-mn-ink">{giverName}</span>
                            <span className="text-mn-mute">→</span>
                            <span className="font-medium text-mn-ink-soft">{receiverName}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-mn-ink-soft text-xs max-w-[200px] truncate">
                          {e.description}
                        </td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_COLORS[e.status] ?? 'bg-mn-elevated text-mn-ink-soft'}`}>
                  ...
                  {entries.length === 0 && (
                    <tr><td colSpan={6} className="px-4 py-12 text-center text-mn-mute">Keine Einträge gefunden</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-mn-elevated rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-mn-mute mb-1">Stunden</label>
