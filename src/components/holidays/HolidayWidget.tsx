...
export function HolidayWidget({
  ...
  if (!loaded) {
    return (
      ...
      >
        <div className="mb-2 h-4 w-32 rounded bg-mn-raised dark:bg-ink-700" />
        <div className="h-8 w-48 rounded bg-mn-raised dark:bg-ink-700" />
      </div>
    )
  }
  ...
  if (holidays.length === 0) {
    return (
      <div
        role="status"
        className={`flex items-center gap-2 rounded-xl border border-white/5 bg-mn-elevated px-4 py-3 text-sm text-mn-ink-soft dark:border-ink-700 dark:bg-ink-800 dark:text-mn-ghost ${className}`}
      >
        <AlertCircle aria-hidden className="h-4 w-4 text-mn-mute" />
  ...
  if (compact) {
    return (
      <div
        className={`flex items-center gap-3 rounded-xl border border-white/5 bg-mn-elevated px-4 py-3 dark:border-ink-700 dark:bg-ink-800 ${className}`}
      ...
      >
        ...
        <div className="flex flex-col leading-tight">
          <span className="text-xs text-mn-mute dark:text-mn-mute">Nächster Feiertag</span>
          <span className="text-sm font-semibold text-mn-ink dark:text-stone-100">
            {next.localName}
          </span>
          <span className="text-xs text-mn-mute dark:text-mn-mute">
  ...
  return (
    ...
    >
      ...
      <ul className="space-y-1.5">
        {holidays.slice(0, 8).map(h => {
          ...
          return (
            ...
            >
              <span className="w-20 text-xs text-mn-mute dark:text-mn-mute">
                {formatDate(h.date)}
              </span>
              <span className="flex-1 text-sm text-mn-ink dark:text-stone-100">
                {h.localName}
              </span>
              {isBridge && (
              ...
              <span className="flex items-center gap-1 text-[11px] text-mn-mute dark:text-mn-mute">
      ...
      <p className="mt-3 text-[11px] text-mn-mute dark:text-mn-mute">
        Daten: Nager.Date
      </p>
    </section>
  )
