      ...
      <div className="bg-mn-elevated rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[80vh] flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
            <Clock className="w-5 h-5 text-mn-bronze" />
            Letzte Anrufe
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="w-6 h-6 border-2 border-mn-bronze border-t-transparent rounded-full animate-spin" />
            </div>
          ) : calls.length === 0 ? (
            ...
            calls.map(call => {
              ...
              return (
                <div key={call.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50">
                  {/* Avatar */}
                  <div className="relative w-10 h-10 rounded-full overflow-hidden bg-mn-bronze/10 flex-shrink-0">
                    {call.partner_avatar ? (
                      <Image src={call.partner_avatar} alt="" fill className="object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-mn-bronze font-semibold text-sm">
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-full text-mn-bronze hover:bg-mn-bronze/5 min-w-[44px] min-h-[44px] flex items-center justify-center"
