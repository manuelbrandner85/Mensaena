      ...
      {[1, 2, 3].map(i => (
        <div key={i} className="animate-pulse flex gap-3 p-3 rounded-xl bg-mn-surface">
          ...
          <div className="flex-1 space-y-1.5 pt-0.5">
            <div className="h-3 bg-mn-raised rounded w-3/4" />
            <div className="h-2.5 bg-mn-raised rounded w-1/2" />
            <div className="h-2.5 bg-mn-raised rounded w-1/3" />
          </div>
        </div>
      ))}
    </div>
...
function ItemCard({ item, color }: { item: Item; color: string }) {
  return (
    <a
      ...
      className="group flex items-start gap-3 p-3 rounded-xl hover:bg-mn-elevated/[0.02] transition-colors"
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.provider && (
          <p className="text-[11px] text-mn-mute truncate mt-0.5">{item.provider}</p>
        )}
        <div className="flex flex-wrap items-center gap-2 mt-1">
          ...
          {item.type === 'course' && (item as Course).category && (
            <span className="text-xs text-mn-mute bg-mn-elevated px-1.5 py-0.5 rounded-full truncate max-w-[120px]">
  ...
  return (
    <div className={cn('bg-mn-elevated rounded-2xl border border-white/5 shadow-soft overflow-hidden', className)}>
      ...
      <div className="relative px-4 pt-4 pb-0">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-mn-ink leading-tight">Bildung in deiner Nähe</h3>
              {plz && <p className="text-[11px] text-mn-mute mt-0.5">PLZ {plz} · 25 km Umkreis</p>}
            </div>
          </div>
        </div>

        {/* Tabs */}
        <div className="flex gap-1 bg-mn-elevated p-1 rounded-xl mb-3">
          {([
            ...
            <button
              ...
              className={cn(
                ...
                tab === key
                  ? 'bg-mn-elevated text-mn-ink shadow-soft'
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 text-xs border border-white/5 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary-300 bg-mn-elevated"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-mn-mute hover:text-mn-ink-soft transition-colors"
            >
      ...
      <div className="px-2 pb-3">
        {loading ? (
          ...
          <div className="px-4 py-6 text-center space-y-1">
            <GraduationCap className="w-8 h-8 text-mn-ghost mx-auto" />
            <p className="text-xs font-medium text-mn-ink-soft">Kein Standort gesetzt</p>
          ...
          <div className="px-4 py-6 text-center space-y-1">
            <BookOpen className="w-8 h-8 text-mn-ghost mx-auto" />
            <p className="text-xs font-medium text-mn-ink-soft">Keine Angebote gefunden</p>
            <p className="text-[11px] text-mn-mute">
      ...
      <div className="px-4 pb-3">
        <p className="text-xs text-mn-mute text-right">
