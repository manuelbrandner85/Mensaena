...
export default function BottomNav({ unreadMessages, unreadNotifications }: BottomNavProps) {
  ...
  return (
    <nav
      className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-lg safe-area-bottom"
      role="navigation"
        ...
        {bottomNavItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.path}
              className={cn(
                'relative flex flex-col items-center justify-center gap-0.5 min-w-[56px] py-1.5 rounded-xl transition-all',
                active
                  ? 'text-primary-600'
