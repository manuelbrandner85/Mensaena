...
export default function EventRideshares({ eventId, currentUserId }: Props) {
  ...
  return (
    <div className="bg-mn-elevated rounded-2xl border border-white/5 p-5 sm:p-6 shadow-soft">
      {/* Header */}
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-mn-mute">Sitze anbieten oder Mitfahrt suchen</p>
          </div>
        </div>
        ...
        <div className="mb-5 border border-primary-100 rounded-2xl p-4 bg-mn-amber/5/30">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="text-xs">
              <span className="block text-mn-ink-soft mb-1.5 font-medium">{role === 'offer' ? 'Freie Sitze' : 'Benötigte Sitze'}</span>
              <input
                type="number" min={1} max={8}
                value={seats}
                onChange={e => setSeats(Math.max(1, Math.min(8, parseInt(e.target.value || '1', 10))))}
                className="w-full h-10 px-3 border border-white/5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent"
              />
            </label>
            <label className="text-xs">
              <span className="block text-mn-ink-soft mb-1.5 font-medium">Abfahrtszeit</span>
              <input
                type="datetime-local"
                value={departureTime}
          ...
          <label className="text-xs block mt-3">
            <span className="block text-mn-ink-soft mb-1.5 font-medium">{role === 'offer' ? 'Abfahrtsort' : 'Startort'}</span>
            <input
              type="text"
              value={fromLocation}
          ...
          <label className="text-xs block mt-3">
            <span className="block text-mn-ink-soft mb-1.5 font-medium">Notiz (optional)</span>
            <textarea
              value={notes}
              ...
              className="w-full px-3 py-2 border border-white/5 rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent"
            />
          </label>
          ...
          <div className="mt-4 flex justify-end gap-2">
            <button
              ...
              className="h-9 px-4 text-xs font-semibold text-mn-ink-soft rounded-xl hover:bg-mn-elevated/5 transition-colors"
            >
              Abbrechen
            </button>
            <button
      ...
      {/* Content */}
      {loading ? (
        <div className="text-xs text-mn-mute py-8 text-center">
          <Loader2 className="w-5 h-5 animate-spin mx-auto mb-2 text-primary-400" />
          Lade Fahrgemeinschaften …
        </div>
      ) : items.length === 0 ? (
        <div className="text-xs text-mn-mute py-8 text-center border border-dashed border-white/5 rounded-2xl">
          <Car className="w-6 h-6 mx-auto mb-2 text-mn-ghost" />
...
function RideColumn({ title, variant, items, currentUserId, onRemove }: {
  ...
  return (
    <div className={`rounded-2xl border p-3 ${isOffer ? 'bg-mn-amber/5/40 border-primary-100' : 'bg-[#4F6D8A]/5 border-[#4F6D8A]/20'}`}>
      ...
      {items.length === 0 ? (
        <p className="text-xs text-mn-mute italic py-2">–</p>
      ) : (
        <ul className="space-y-2">
          {items.map(item => {
            ...
            const dep = item.departure_time
              ...
              : null
            return (
              <li key={item.id} className="bg-mn-elevated rounded-xl border border-white/5 p-3 relative shadow-soft hover:shadow-card transition-shadow">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-semibold text-mn-ink truncate">{name}</p>
                    <p className="text-[11px] text-mn-ink-soft mt-0.5 font-medium">
                      {item.seats} {item.seats === 1 ? 'Sitz' : 'Sitze'}
                    </p>
                    {item.from_location && (
                      <p className="text-[11px] text-mn-mute mt-0.5 flex items-center gap-1">
                    ...
                    {item.notes && (
                      <p className="text-[11px] text-mn-mute mt-1 italic">„{item.notes}"</p>
                    )}
                  </div>
