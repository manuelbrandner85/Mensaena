...
import {
  ...
  CheckCircle2, HelpCircle, ArrowLeft,
} from 'lucide-react'
...
interface ModulePageProps {
  ...
  emptyText?: string
  /** @deprecated Wird nicht mehr lokal verwendet – der zentrale Create-Flow handhabt Anonymität. */
...
export default function ModulePage({
  ...
  emptyText, allowAnonymous: _allowAnonymous, filterCategory, children,
  ...
  const createUrl = useMemo(() => {
    ...
    const firstType = createTypes[0]?.value
    if (firstType) params.set('type', firstType)
    const paramStr = params.toString()
    return `/dashboard/create${paramStr ? `?${paramStr}` : ''}`
  }, [moduleKey, createTypes])
        ...
        examplePosts && examplePosts.length > 0 ? (
          <div className="space-y-4">
              ...
              {examplePosts.map((ex, i) => (
                <button
                  ...
                  onClick={() => {
                    const params = new URLSearchParams()
                    if (moduleKey) params.set('module', moduleKey)
                    params.set('type', ex.type)
                    params.set('category', ex.category)
                    params.set('title', ex.title)
            ...
            <div className="flex justify-center gap-3 flex-wrap pt-2">
              <Link href={createUrl} className="btn-primary inline-flex items-center gap-2">
                <Plus className="w-4 h-4" />
                Jetzt erstellen
              </Link>
              {activeTab !== 'alle' && (
